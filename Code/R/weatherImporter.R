...
### get previous day
date <- as.Date(substr(Sys.time(),1,10)) - 1

url <- paste(part,token,date, sep = "")
...
system.time(
  ...
  {
    doc <- readHTMLTable(url[x], which = 2, skip.rows = c(1,2), 
                         colClasses = col.type, encoding = "utf-8") %>% 
      set_colnames(., title)
    
    doc.tag <- readHTMLTable(url[x], which = 1, header = c(""), encoding = "UTF-8")
...
### insert to mongoDB
m.weather <- mongo(collection = "weather", db = "bike")
m.weather$insert(myTable)
